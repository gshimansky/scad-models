...
module back_wall() {
    translate([0, -wall_thickness / 2, 0])
        cube(size = [board_size + board_extra_size * 2 + bolt_well_diameter_wide * 2, wall_thickness, board_size + board_extra_size * 2], center = true);
}

module screw_well() {
    rotate(a=[90, 0, 0]) difference() {
        cylinder($fn = 24, h = screw_depth, d1 = screw_wall_thickness * 2 + screw_diameter, d2 = wall_thickness * 2 + screw_diameter);
...
module lens_screw_well() {
    rotate(a=[90, 0, 0])
        cylinder($fn = 24, h = screw_depth, d = wall_thickness * 2 + lens_screw_head_diameter);
}
